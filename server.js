const path=require('path'),http=require('http'),express=require('express'),{WebSocketServer}=require('ws');
const app=express(),root=process.pkg?path.dirname(process.execPath):__dirname;app.use(express.static(root,{extensions:['html']}));app.get('/',(q,r)=>r.sendFile(path.join(root,'index.html')));app.get('/health',(q,r)=>r.json({ok:true,game:'The Rake',players:players.size}));
const srv=http.createServer(app),wss=new WebSocketServer({server:srv,path:'/ws'}),players=new Map();let nextId=1,clockTime=20,last=Date.now();
