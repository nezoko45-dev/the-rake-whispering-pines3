...
wss.on('connection',ws=>{const id=Math.random().toString(36).slice(2,10),p={id,ws,x:0,y:0,z:20,yaw:0,hp:100,charging:false};players.set(id,p);ws.send(JSON.stringify({type:'welcome',id,player:{id:p.id,x:p.x,y:p.y,z:p.z,yaw:p.yaw,hp:p.hp,charging:false}}));ws.on('message',raw=>{let m;try{m=JSON.parse(raw)}catch{return}if(m.type==='input'){p.yaw=+m.yaw||0;p.charging=!!m.charging;const l=Math.hypot(m.x||0,m.z||0)||1,s=p.charging?10:6;p.x+=m.x/l*s*.05;p.z+=m.z/l*s*.05}if(m.type==='stunstick'){p.charging=!!m.charging;const q=d(p,rake);if(q>9)return;const dx=rake.x-p.x,dz=rake.z-p.z,l=Math.hypot(dx,dz)||1,face=(Math.sin(p.yaw)*dx-Math.cos(p.yaw)*dz)/l;if(p.charging&&face>.82){rake.state='parry';rake.stun=Date.now()+350;p.hp=Math.max(0,p.hp-10);ws.send(JSON.stringify({type:'stunstick'}));return}rake.hp=Math.max(0,rake.hp-45);rake.stun=Date.now()+2500}});ws.on('close',()=>players.delete(id))});
