...
const server=http.createServer((req,res)=>{if(req.url==='/'||req.url==='/index.html'){fs.readFile(path.join(__dirname,'index.html'),(err,data)=>{if(err){res.writeHead(500);res.end('Game file unavailable');return}res.writeHead(200,{'Content-Type':'text/html; charset=utf-8'});res.end(data)});return}if(req.url==='/health'){res.writeHead(200,{'Content-Type':'application/json'});res.end(JSON.stringify({ok:true,players:wss.clients.size}));return}res.writeHead(404);res.end('Not found')});
