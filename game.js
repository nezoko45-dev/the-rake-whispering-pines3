...
const camera=new THREE.PerspectiveCamera(72,innerWidth/innerHeight,.05,500),renderer=new THREE.WebGLRenderer({antialias:false,powerPreference:'low-power',stencil:false,depth:true,precision:'mediump'});
renderer.setPixelRatio(1);renderer.setSize(innerWidth,innerHeight);renderer.shadowMap.enabled=false;document.body.appendChild(renderer.domElement);
scene.add(new THREE.HemisphereLight(0x9aa6b8,0x16120d,1.35));const moon=new THREE.DirectionalLight(0xaab8ff,1.3);moon.position.set(30,60,10);scene.add(moon);
...
loader.load('/assets/rake/rake.glb',g=>{
  rake=g.scene;
  rake.traverse(o=>{if(o.isMesh){o.castShadow=false;o.receiveShadow=false;if(o.material)o.material.needsUpdate=true}});
