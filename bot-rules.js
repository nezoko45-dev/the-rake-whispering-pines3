(()=>{
  ...
  function chooseTarget(){const nearby=allTargets().filter(t=>dist(rake,t.ref)<=100).sort((a,b)=>dist(rake,a.ref)-dist(rake,b.ref));if(currentTarget&&alive(currentTarget.ref)&&dist(rake,currentTarget.ref)<=100&&targetHits<2)return currentTarget;const next=nearby.find(t=>!lastTarget||t.ref!==lastTarget.ref)||nearby[0]||null;currentTarget=next;targetHits=0;return next}
  function registerHit(t){if(!t||!alive(t.ref))return;if(t.kind==='player')hurtPlayer(25);else{t.ref.health=Math.max(0,t.ref.health-25);if(t.ref.health<=0){t.ref.dead=true;t.ref.respawn=2.2;banner(t.ref.name+' was taken by THE RAKE');if(currentTarget&&currentTarget.ref===t.ref)currentTarget=null;updateCount()}}targetHits++;if(targetHits>=2){lastTarget=t;currentTarget=null;targetHits=0}else if(!alive(t.ref)){lastTarget=t;currentTarget=null;targetHits=0}}
