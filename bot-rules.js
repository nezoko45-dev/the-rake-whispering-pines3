(()=>{
  ...
  const bots=[];
  const makeBot=i=>{const a=i*Math.PI*2/botCount+Math.random()*.4,d=170+Math.random()*110;return{id:i,name:names[i],x:Math.max(60,Math.min(WW-60,spawn.x+Math.cos(a)*d)),y:Math.max(60,Math.min(WH-60,spawn.y+Math.sin(a)*d)),r:11,health:100,maxHealth:100,dead:false,respawn:0,wander:null,wanderTimer:0,joined:false,joinTimer:0}};
  for(let i=0;i<botCount;i++){const b=makeBot(i);b.joined=i<2||Math.random()<.55;b.joinTimer=b.joined?(25+Math.random()*45):(5+Math.random()*20);bots.push(b)}
  ...
  function spawnBot(b){const n=makeBot(b.id);b.x=n.x;b.y=n.y;b.health=100;b.dead=false;b.joined=true;b.wander=null;b.wanderTimer=0;b.joinTimer=25+Math.random()*45;banner(b.name+' joined the game')}
  function despawnBot(b){b.joined=false;b.dead=false;b.health=100;b.wander=null;b.wanderTimer=0;b.joinTimer=12+Math.random()*28;if(currentTarget&&currentTarget.ref===b)currentTarget=null;if(lastTarget&&lastTarget.ref===b)lastTarget=null;banner(b.name+' left the game')}
  function updateBots(dt){
    for(let i=0;i<bots.length;i++){
      const b=bots[i];
      if(!b.joined){b.joinTimer-=dt;if(b.joinTimer<=0)spawnBot(b);continue}
      if(b.dead){b.respawn-=dt;if(b.respawn<=0){spawnBot(b)}continue}
      b.joinTimer-=dt;
      if(b.joinTimer<=0&&bots.filter(alive).length>2&&Math.random()<.35){despawnBot(b);continue}
      b.wanderTimer-=dt;
      if(!b.wander||b.wanderTimer<=0){b.wander={x:rand(80,WW-80),y:rand(80,WH-80)};b.wanderTimer=2.5+Math.random()*4}
      botMove(b,b.wander,8+Math.random()*3,dt)
    }
  }
  ...
  $('connection').textContent='AI Bots';$('playerCount').textContent=String(bots.filter(b=>b.joined).length+1);
  const legend=document.querySelector('.legend');if(legend)legend.textContent='Dark circles are bushes • 100 studs = active range • AI survivors join and leave like players';
  function drawBots(){const scale=Math.min(W/1100,H/700),viewW=W/scale,viewH=H/scale,cameraX=Math.max(0,Math.min(WW-viewW,player.x-viewW/2)),cameraY=Math.max(0,Math.min(WH-viewH,player.y-viewH/2));x.save();x.scale(scale,scale);x.translate(-cameraX,-cameraY);for(const b of bots){if(!b.joined||b.dead)continue;x.fillStyle='#6e9fff';x.beginPath();x.arc(b.x,b.y,b.r,0,Math.PI*2);x.fill();x.fillStyle='#e7eefb';x.font='11px system-ui';x.textAlign='center';x.fillText(b.name,b.x,b.y-18);x.fillStyle='#1b2420';x.fillRect(b.x-16,b.y+15,32,4);x.fillStyle='#69cf7b';x.fillRect(b.x-16,b.y+15,32*(b.health/b.maxHealth),4)}x.restore();$('playerCount').textContent=String(bots.filter(b=>b.joined).length+1)}
